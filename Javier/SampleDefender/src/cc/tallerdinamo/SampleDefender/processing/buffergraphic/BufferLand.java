...
public class BufferLand extends BufferDraw {
	...
	private float bordeSup;
	private float bordeInf;
	
	public BufferLand(PApplet _p5, float[] bufferArrayIn) {
		super(_p5, bufferArrayIn);
		roofLand = new ArrayList<Float>(p5.displayWidth);
		groundLand = new ArrayList<Float>(p5.displayWidth);
		portales = new ArrayList<Portal>(getNumOfSections()); //cantidad de portales segundo diviçāo de samples
		...
		noiseTime = 0;
		bordeSup = -landHeight/2;
		bordeInf = landHeight/2;
		
		Portal.setBordes(bordeSup, bordeInf);
		for (int i = 0 ; i < getNumOfSections() ; i++) {
			portales.add(new Portal (_p5));
		}
		getRoofGroundLand();
...
//		p5.strokeWeight(1);
		...
		int div = (int)(visibleSectionWidth / getSplitSampleWidth()); //faixa de samples visiveis / largura em samples de cada corte
		if (div <= 1) {
			div = 1;
		}
		float sectionWidth = p5.displayWidth / div;
		
		//Desenho do primeir retângulo de fundo 
		...
		p5.noStroke();
		p5.rect(0, bordeSup, sectionWidth, landHeight);//desenho a partir do canto superior esquerdo
